  ...
  return (
    <>
      <div className="semesters lg:px-28 px-4 py-20 md:py-28 font-jost">
        <h1 className="text-7xl pb-8 lg:px-6 font-jost font-extrabold">
        ...
        <div className="wrapper flex flex-wrap">
          {subjectNotes.map((item) => {
            ...
            const truncatedName =
              name.length > 20 ? `${name.substring(0, 20)}...` : name; // Truncate name if it exceeds 20 characters
            return (
              <div
                key={$id}
                className="semester bg-[#D7D9DD] shadow-2xl shadow-black p-5 rounded-2xl mb-6 mx-2 lg:mx-5 flex flex-col justify-center"
              >
                <Link target="_blank" href={`${url}`}>
                  <Image
                    title="Click to view"
                    className="cursor-pointer  mb-3 lg:w-52 hover:scale-105 transition-all duration-300"
                    src={`/images/extensions/${ext}.svg`}
                    width={300}
                    height={300}
                    alt="subjectFolder"
                  />
